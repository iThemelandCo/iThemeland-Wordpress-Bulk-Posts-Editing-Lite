...
class Post
{
    ...
    {
        $types = [
            'post' => esc_html__('Post', WPBEL_NAME),
            'page' => esc_html__('Page', WPBEL_NAME),
        ];
    ...
    public function get_post_type_name(string $post_type)
    {
        switch ($post_type) {
            case 'post':
            case 'page':
    ...
    public function get_post_ids_by_custom_query($join, $where, array $types_in = ['post'])
    {
        global $wpdb;
        $types = "";
        if (!empty($types_in)) {
    ...
    public function get_posts_list(array $post_types, array $data = [], int $active_page = 1)
    {
    ...
    public function update(array $post_ids, array $data)
    {
        if (empty($post_ids)) {
            return false;
        }
    ...
    public function get_post_fields(\WP_Post $post_object): array
    {
    ...
    public function delete(int $post_id, string $deleteType)
    {
    ...
    private function parse_value(\WP_Post $post, $value)
    {
    ...
    private function update_sticky_post(int $post_id, string $value)
    {
        $sticky_posts = $this->get_sticky_posts();
        if ($value == 'yes') {
