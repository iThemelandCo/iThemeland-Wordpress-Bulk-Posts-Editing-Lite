<table id="wpbel-items-list">
        ...
        <tr>
            <?php if (isset($show_id_column) && $show_id_column === true) : ?>
                ...
                <th class="wpbel-td70 <?php echo ($sticky_first_columns == 'yes') ? 'wpbel-td-sticky wpbel-td-sticky-id' : ''; ?>">
                    <input type="checkbox" class="wpbel-check-item-main" title="<?php esc_html_e('Select All', WPBEL_NAME); ?>">
                    <label data-column-name="id" class="wpbel-sortable-column"><?php esc_html_e('ID', WPBEL_NAME); ?><span class="wpbel-sortable-column-icon"><?php echo sprintf('%s', $sortable_icon); ?></span></span>
                </th>
            <?php endif; ?>
            <?php if (!empty($next_static_columns)) : ?>
                <?php foreach ($next_static_columns as $static_column) : ?>
                    ...
                    ?>
                    <th data-column-name="<?php echo esc_attr($static_column['field']) ?>" class="wpbel-sortable-column wpbel-td120 <?php echo ($sticky_first_columns == 'yes') ? 'wpbel-td-sticky wpbel-td-sticky-title' : ''; ?>"><?php echo esc_attr($static_column['title']); ?><span class="wpbel-sortable-column-icon"><?php echo sprintf('%s', $sortable_icon); ?></span></th>
                <?php endforeach; ?>
            <?php endif; ?>
    ...
    <tbody>
        <?php if (!empty($items_loading)) : ?>
            <tr>
                <td colspan="8" class="wpbel-text-alert"><?php esc_html_e('Loading ...', WPBEL_NAME); ?></td>
            </tr>
        ...
        <?php else : ?>
            <tr>
                <td colspan="8" class="wpbel-text-alert"><?php esc_html_e('No Data Available!', WPBEL_NAME); ?></td>
            </tr>
        <?php endif; ?>
    </tbody>
    <tfoot>
